...
def test_emailextractor_extract_single_email(email_extractor):
    ...
    assert extracted_data[0] == "admin@example.com", "Email extracted should match: admin@example.com"
...
def test_emailextractor_extract_email_with_plus_sign(email_extractor):
    ...
    assert extracted_data[0] == "user+tag@example.com", "Email extracted should match: user+tag@example.com"
...
def test_emailextractor_extract_multiple_email_patterns(email_extractor):
    ...
    assert "dev+backend@tech-startup.io" in extracted_data, "Should extract dev+backend@tech-startup.io"
...
def test_urlextractor_extract_http_url(url_extractor):
    ...
    assert extracted_data[0] == "http://example.com", "URL extracted should match: http://example.com"
...
def test_urlextractor_extract_https_url(url_extractor):
    ...
    assert isinstance(extracted_data, list), "Data structure should be a List"
    assert len(extracted_data) == 1, "Should return 1 URL occurrence"
    assert extracted_data[0] == "https://secure.example.com", "URL extracted should match: https://secure.example.com"
...
def test_urlextractor_extract_ftp_url(url_extractor):
    ...
    assert isinstance(extracted_data, list), "Data structure should be a List"
    assert len(extracted_data) == 1, "Should return 1 URL occurrence"
    assert extracted_data[0] == "ftp://ftp.example.com/file.zip", "URL extracted should match: ftp://ftp.example.com/file.zip"
...
def test_urlextractor_extract_url_with_path(url_extractor):
    ...
    assert isinstance(extracted_data, list), "Data structure should be a List"
    assert len(extracted_data) == 1, "Should return 1 URL occurrence"
    assert extracted_data[0] == "https://docs.example.com/api/v1/documentation", "URL extracted should match: https://docs.example.com/api/v1/documentation"
...
def test_customextractor_create_with_single_pattern():
    ...
    assert len(phone_extractor._regex_pattern_list) == 1, "Should have 1 pattern"
...
def test_customextractor_create_with_multiple_patterns():
    ...
    assert phone_extractor.name == "phone", "Extractor name should be 'phone'"
    assert len(phone_extractor._regex_pattern_list) == 2, "Should have 2 patterns"
...
def test_customextractor_extract_with_duplicates():
    ...
    assert extracted_data.count("555-123-4567") == 2, "Should return 2 occurrences for phone '555-123-4567'"
    assert extracted_data.count("555-987-6543") == 1, "Should return 1 occurrence for phone '555-987-6543'"
...
def test_customextractor_invalid_regex_pattern():
    ...
    assert "Invalid regex pattern" in str(exc_info.value), "Should raise ValueError with proper message"
...
def test_customextractor_add_patterns_dynamically():
    ...
    assert len(phone_extractor._regex_pattern_list) == 1, "Should have 1 pattern initially"
    phone_extractor.add_patterns([r"\(\d{3}\)\s*\d{3}-\d{4}"])
    assert len(phone_extractor._regex_pattern_list) == 2, "Should have 2 patterns after adding"
...
def test_customextractor_empty_pattern_list():
    ...
    assert len(empty_extractor._regex_pattern_list) == 0, "Should have 0 patterns"
